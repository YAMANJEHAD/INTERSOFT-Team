...
if not df_count.empty and "Employee" in df_count.columns:
    if st.session_state.user_role_type == "Admin":
        df_filtered = df_count
    else:
        df_filtered = df_count[df_count['Employee'] == st.session_state.user_role]
else:
    df_filtered = pd.DataFrame()

total_tasks = len(df_filtered)
completed_tasks = df_filtered[df_filtered['Status'] == '✅ Completed'].shape[0] if 'Status' in df_filtered.columns else 0
in_progress_tasks = df_filtered[df_filtered['Status'] == '🔄 In Progress'].shape[0] if 'Status' in df_filtered.columns else 0
