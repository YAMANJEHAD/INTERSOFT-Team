...
def render_download_tasks():
    ...
    if not user_tasks.empty:
        ...
        file_name = f"{current_user}tasks{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.xlsx"
        if category != "All":
            file_name = f"{current_user}tasks{category}{start.strftime('%Y%m%d')}{end.strftime('%Y%m%d')}.xlsx"
        if priority != "All":
            file_name = f"{current_user}tasks{category}{priority}{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.xlsx"
    ...
    if st.session_state.user_role_type == "Admin":
        ...
        with st.expander("📅 Task Management", expanded=False):
            if not df_all.empty and 'Employee' in df_all.columns:
                ...
                file_name = f"tasks_{selected_user}{start.strftime('%Y%m%d')}{end.strftime('%Y%m%d')}.xlsx"
                if category != "All":
                    file_name = f"tasks_{selected_user}{category}{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.xlsx"
                if priority != "All":
                    file_name = f"tasks_{selected_user}{category}{priority}{start.strftime('%Y%m%d')}{end.strftime('%Y%m%d')}.xlsx"
