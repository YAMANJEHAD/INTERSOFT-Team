...
if not df_user.empty and all(col in df_user.columns for col in ["Status"]):
...
with tab2:
    ...
    if not df_user.empty:
        ...
        selected_key = st.selectbox("📋 Choose Task", list(task_dict.keys()))
        task_id = task_dict[selected_key]
        task_data = df_user[df_user["TaskID"] == task_id].iloc[0]

        with st.form("edit_form"):
            c1, c2 = st.columns(2)
            with c1:
                shift = st.selectbox("🕒 Shift", SHIFTS, index=SHIFTS.index(task_data["Shift"]))
                date = st.date_input("📅 Date", datetime.strptime(task_data["Date"], '%Y-%m-%d'))
                dept = st.selectbox("🏢 Department", ["FLM", "Tech Support", "CRM"], index=["FLM", "Tech Support", "CRM"].index(task_data["Department"]))
            with c2:
                cat = st.selectbox("📂 Category", CATEGORIES, index=CATEGORIES.index(task_data["Category"]))
                stat = st.selectbox("📌 Status", STATUSES, index=STATUSES.index(task_data["Status"]))
                prio = st.selectbox("⚠️ Priority", PRIORITIES, index=PRIORITIES.index(task_data["Priority"]))
            desc = st.text_area("🗒 Task Description", task_data["Description"])
...
with tab3:
    if not df_user.empty:
        ...
        st.info("ℹ️ Add tasks to view analytics.")
...
col1, col2, col3, col4 = st.columns(4)
col1.markdown(f"<div class='overview-box'>Total Tasks<br><span>{len(df_user)}</span></div>", unsafe_allow_html=True)
col2.markdown(f"<div class='overview-box'>Completed<br><span>{df_user[df_user['Status']=='✅ Completed'].shape[0]}</span></div>", unsafe_allow_html=True)
col3.markdown(f"<div class='overview-box'>In Progress<br><span>{df_user[df_user['Status']=='🔄 In Progress'].shape[0]}</span></div>", unsafe_allow_html=True)
col4.markdown(f"<div class='overview-box'>Not Started<br><span>{df_user[df_user['Status']=='⏳ Not Started'].shape[0]}</span></div>", unsafe_allow_html=True)
...
with tab2:
    st.subheader("✏️ Edit or Delete Task")
    if not df_user.empty:
        options = {f"{row['Description'][:30]}... ({row['Date']})": row['TaskID'] for _, row in df_user.iterrows()}
        ...
        with st.form("edit_form"):
            ...
            with c1:
                ...
                department = st.selectbox("🏢 Department", ["FLM", "Tech Support", "CRM"], index=["FLM", "Tech Support", "CRM"].index(selected_task["Department"]))
            with c2:
            ...
            desc = st.text_area("🗒 Task Description", selected_task["Description"], height=100)

            col_save, col_del = st.columns(2)
            update = col_save.form_submit_button("💾 Update Task")
            delete = col_del.form_submit_button("🗑 Delete Task")

            if update:
                for idx, task in enumerate(st.session_state.timesheet):
                    if task["TaskID"] == selected_id:
                        st.session_state.timesheet[idx] = {
                            "TaskID": selected_id,
                            "Employee": st.session_state.user_role,
                            "Date": date.strftime('%Y-%m-%d'),
                            "Day": calendar.day_name[date.weekday()],
                        ...
                        st.rerun()
            if delete:
                st.session_state.timesheet = [task for task in st.session_state.timesheet if task["TaskID"] != selected_id]
                st.warning("🗑 Task deleted.")
                st.rerun()
    else:
        st.info("ℹ️ No tasks to edit. Add one first.")
...
with tab3:
    if not df_user.empty:
        ...
        st.plotly_chart(px.histogram(df_user, x="Date", color="Status", title="Tasks Over Time"), use_container_width=True)
        st.plotly_chart(px.pie(df_user, names="Category", title="Category Breakdown"), use_container_width=True)
        st.plotly_chart(px.bar(df_user, x="Priority", color="Priority", title="Priority Levels"), use_container_width=True)
        ...
        output = BytesIO()
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            df_user.to_excel(writer, sheet_name="Tasks", index=False)
            worksheet = writer.sheets["Tasks"]
            for i, col in enumerate(df_user.columns):
                worksheet.set_column(i, i, 18)
        st.download_button("⬇️ Download Excel", data=output.getvalue(), file_name="FLM_Tasks.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    else:
        st.info("ℹ️ No tasks to analyze.")
